...
public class Test {
	...
	public static boolean isAndTrue(int a,int b)
	{
		boolean bRet = false;
		bRet = ((a&b)==1)?true:false;
		return bRet;
	}
	...
	public static void main(String argv[]) throws NativeException,
			IllegalAccessException, InterruptedException {
		System.load("E:\\CC301SDK6.4\\bin\\qnviccub.dll");// InterfaceFun是dll文件这个在初始化时应该引入
	...
	//	int iSn = BriSDKLib.QNV_DevInfo(0, BriSDKLib.QNV_DEVINFO_GETSERIAL);// 取得序列号
...
//					evtCall.getCallbackAddress(), null, 0);
		...
		while (true) {
			try {
				...
				if (ls.compareTo("quit") == 0) {
					System.out.print("\n你真的想退出吗?(y/n)");
					size = System.in.read(buffer, 0, 255);
					ls = new String(buffer, 0, size);
					ls = ls.trim().toLowerCase();
					if (ls.compareTo("y") == 0)
						break;
				} else if (ls.compareTo("help") == 0) {
					printHelp();
				} else if (ls.startsWith("loc")) {
					String[] strline = ls.split("(\\s* \\s*)|(\\s*and\\s*)");
					if (strline.length != 2) {
						System.out.println("命令格式错误，这里是两个参数!");
					} else {
						StringBuffer szBuf = new StringBuffer(128);
						BriSDKLib.QNV_Tool(BriSDKLib.QNV_TOOL_LOCATION, 0,
								strline[1], "0", szBuf, 128);
						System.out.println("测试" + strline[1] + "的地区为:" + szBuf);
					}
				} else if (ls.startsWith("dial")) {
					String[] strline = ls.split("(\\s* \\s*)|(\\s*and\\s*)");
					if (strline.length != 3) {
						System.out
								.println("命令格式错误，这里是3个参数!如dial 0 9,1323343434 其中9是出局号");
					} else {
						boolean bFlag = true;
				...
				} else if (ls.startsWith("hang")) {
					String[] strline = ls.split("(\\s* \\s*)|(\\s*and\\s*)");
					if (strline.length != 2) {
						System.out.println("命令格式错误，这里是2个参数!如:hang 0");
					} else {
						int iCh = Integer.parseInt(strline[1]);
						BriSDKLib.QNV_General(iCh,
						...
						System.out.println("通道 " + iCh + "电话已挂机");
					}
				}
				 else if (ls.startsWith("setspkmic")) {
						String[] strline = ls.split("(\\s* \\s*)|(\\s*and\\s*)");
						if (strline.length != 3) {
							System.out
									.println("命令格式错误，这里是3个参数!如setspkmic 0 1(0是关闭，1是打开ia)");
						} else {
							boolean bFlag = true;
							int iCh = Integer.parseInt(strline[1]);
							int iFlag = Integer.parseInt(strline[2]);
								// BriSDKLib.QNV_SetDevCtrl(iCh,BriSDKLib.QNV_CTRL_PLAYTOLINE,1);//自动打开播放语音到LINE,驱动会自动根据摘挂机状态打开关闭控制
								BriSDKLib.QNV_SetDevCtrl(iCh,
										BriSDKLib.QNV_CTRL_DOLINETOSPK, iFlag);//
								BriSDKLib.QNV_SetDevCtrl(iCh,BriSDKLib.QNV_CTRL_DOMICTOLINE ,iFlag);
							if(iFlag == 1)
								System.out.println("耳麦通道打开");
							else
								System.out.println("耳麦通道关闭");
						}
					} 				
				else {
					printHelp();
		...
		System.out.println("请稍后,关闭设备正在退出......");
		// 关闭所有设备
		iRet = BriSDKLib.QNV_CloseDevice(BriSDKLib.ODT_ALL, 0);
		if (iRet > 0)
			System.out.println("设备已关闭");
		else
			System.out.println("设备关闭异常");
